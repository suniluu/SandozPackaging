...
export default class AgreementHomePage extends LightningElement {
  ...
  handleTabClick(event) {
    ...
    if (parseInt(selectedStep) > parseInt(this.currentStep)) {
      ...
      if (canNavigate) {
        ...
        console.error(
          "Validation failed. Please complete all required fields."
        );
      }
    } else {
      this.currentStep = selectedStep;
  ...
  validateCurrentStep() {
    const headerComponent = this.template.querySelector(
  ...
  handleFieldChange(event) {
    ...
    console.log("Field Change Detected:", fieldName, "=", fieldValue);
    ...
    console.log(
      "Updated fieldDataWithLabels in Parent:",
      JSON.stringify(this.fieldDataWithLabels)
    );
  }
  ...
  handleFieldValuesChange(event) {
    ...
    console.log(
      "jj Updated Field Values in Parent:",
      JSON.stringify(this.fieldValues)
    );
  }
      ...
      .then((result) => {
        ...
        if (this.discountdataprod.length > 0) {
          for (let i = 0; i < productDataWithIndex.length; i++) {
            ...
            for (let j = 0; j < this.discountdataprod.length; j++) {
              if (
                productDataWithIndex[i].productId ===
                this.discountdataprod[j].productId
              ) {
                found = true;
                break;
              }
        ...
        if (this.discountdataprod.length > 0) {
          ...
          this.initialData = this.initialData
            ? this.initialData.concat(productDataWithIndex)
            : productDataWithIndex;
          this.initialRecords = this.productdata;
          this.discountdataprod = productDataWithIndex.concat(
            this.discountdataprod
          );
        } else {
  ...
  deletedata(event) {
    ...
    if (this.deleteRecordData.length > 0) {
      ...
      if (this.preSelecteCatelog) {
        const deletedIds = this.deletedRecord.map((item) => item.recordId);
        this.preSelectedRows = this.preSelecteCatelog.filter(
          (item) => !deletedIds.includes(item)
        );
        this.preSelecteCatelog = this.preSelectedRows;
      }
      if (this.preSelectedFast) {
        const deletedIds = this.deleteRecordData.map((item) => item.recordId);
        this.preFastSelectedRows = this.preSelectedFast.filter(
          (item) => !deletedIds.includes(item)
        );
