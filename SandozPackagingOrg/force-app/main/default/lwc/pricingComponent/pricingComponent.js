...
export default class PricingComponent extends LightningElement {
  ...
  @wire(getColumns, { columnData: "Pricing_Table" }) wiredColumns({
    data,
    error
  }) {
    if (data) {
  ...
  loadPricingData() {
    ...
    getPricingRecords({
      recId: this.recId,
      fieldsData: JSON.stringify(this.fields)
    })
      .then((result) => {
  ...
  handleRowSelection(event) {
    ...
    this.preSelectedRows = event.detail.selectedRows.map((item) => item.Id);
    let selectedData = {
  ...
  addCustomStyle(selector, backgroundColor) {
    ...
    style.innerText =
      ".pricing .slds-table thead th:nth-child(" +
      (this.columnIndex + 2) +
      ") " +
      selector +
      " { background-color: " +
  ...
  renderedCallback() {
    if (this.template.querySelector(".pricing")) {
      ...
      style5.innerText =
        ".pricing .dt-outer-container .slds-table_header-fixed_container .slds-scrollable_y {height: 20.5rem;}";
      this.template.querySelector(".pricing").appendChild(style5);

      if (!this.componentLoaded) {
        const style1 = document.createElement("style");
        style1.innerText =
          ".pricing .dt-outer-container .slds-table_header-fixed_container {background-color : white}";
