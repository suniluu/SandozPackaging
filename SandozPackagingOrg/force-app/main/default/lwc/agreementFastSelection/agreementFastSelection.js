...
export default class AgreementFastSelection extends LightningElement {
    ...
    loadCatelogData() {
        ...
        const productId ='';
        getPricingRecords({ recId: this.recId,productId: productId,fieldsData : JSON.stringify(this.fields)})
        .then(result => {
    ...
    addRows() {
        ...
        for (var row of this.fieldDataWithLabels) {
           if (row.isPriceList != '' && row.relatedId == '' || row.relatedId == null && row.isPriceList == true) {
                pricelistId = row.priceListId;
                valuesofpl =row.value;
               // break;
            }else if(row.isPriceList != '' && row.relatedId != '' && row.isPriceList == true){
                pricelistId = row.relatedId;
               // break;
            }
        }
        console.log('Pack fast order pricelistId 214 :: '+pricelistId);
         console.log('Pack fast order valuesofpl 219 :: '+valuesofpl);
        for (let i = 1; i <= this.numRows; i++) {
            var rowData = {"recordId": this.rowIndex,"isDisabled":true,"isFamily":false,"options":this.options,"selectedValue":"Product Name","priceList" : pricelistId};
